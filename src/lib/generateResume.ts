...
import { jobs, resumeIdentity, resumeSocialLinks, resumeSkills } from '../data/resume';
import type { Reference } from '../data/resume';
...
export async function generateResume(options: {
  ...
  const { references, phone } = options;
  ...
  draw(resumeIdentity.name, ML, y, bold, 22, BLACK);
  draw(resumeIdentity.title, ML, y - 18, reg, 9, GRAY);

  const contactItems = [
    ...(phone ? [{ label: phone, href: `tel:${phone}` }] : []),
    ...Object.values(resumeSocialLinks),
  ];
  const CONTACT_LINE_H = 12;
  contactItems.forEach(({ label, href }, i) => {
    const cy = headerTopY - i * CONTACT_LINE_H;
    const x = rightDraw(label, ML + W, cy, reg, 8.5, LINK);
    linkAnnot(href, x, cy, reg.widthOfTextAtSize(sanitize(label), 8.5), 8.5);
  });
  ...
  // ── Skills ────────────────────────────────────────────────
  section('SKILLS');
  const skillLines = wrapWords([...resumeSkills].join(' / '), reg, 8.5, W);
  ...
  return doc.save() as Promise<Uint8Array>;
}
