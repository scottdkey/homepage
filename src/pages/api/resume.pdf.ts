...
const SKILLS = [
  'TypeScript', 'React', 'Next.js', 'Node.js', 'Fastify', 'PostgreSQL',
  'Redis', 'Kafka', 'RabbitMQ', 'Kubernetes',
  'GCP (GKE, Cloud SQL, GCS, Artifact Registry)', 'AWS', 'Docker', 'Stripe', 'Shopify',
];
...
export const GET: APIRoute = async () => {
  ...
  function rightDraw(text: string, rightX: number, yy: number, font: typeof reg, size: number, color = GRAY): number {
  ...
  skillLines.forEach(sl => { draw(sl, ML, y, reg, 8.5, GRAY); y -= 12; });
  ...
  if (references.length > 0 && y > BOTTOM + 60) {
    ...
    const INFO_W   = PAIR_W * 0.52;   // left sub-col: name/title/company
